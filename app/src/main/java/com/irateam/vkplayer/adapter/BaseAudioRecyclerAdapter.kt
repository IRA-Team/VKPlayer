...
abstract class BaseAudioRecyclerAdapter<A : Audio, VH : RecyclerView.ViewHolder> :
    ...
    open fun removeChecked() {
        checkedAudios.toList().forEach {
            ...
            audios -= audios[index]
            removeViewByPosition(index)
        }
        checkedAudios.clear()
    }

    protected fun removeViewByPosition(position: Int) {
    ...
    @Subscribe
    fun onStopEvent(e: PlayerStartEvent) {
        notifyEvent(e)
    }

    protected open fun notifyEvent(e: PlayerEvent) {
